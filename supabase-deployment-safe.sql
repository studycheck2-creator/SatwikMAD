-- =====================================================
-- SAFE SUPABASE BACKEND DEPLOYMENT SCRIPT
-- =====================================================
...
-- Drop problematic triggers first if they exist
...
-- =====================================================
-- STEP 2: ADD COLUMNS TO EXISTING TABLES (MIGRATION)
-- =====================================================

-- If old profiles table exists, add missing columns
DO $$
BEGIN
  ...
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'profiles' AND table_schema = 'public') THEN
    BEGIN
      ...
      ALTER TABLE profiles ADD COLUMN IF NOT EXISTS role TEXT DEFAULT 'member' CHECK (role IN ('member', 'officer', 'admin', 'super_admin'));
...
SELECT '✅ Safe Supabase Backend Deployment Complete!' as status,
       'All tables created, indexes added, RLS enabled, and sample data inserted' as details;